//! tenso-device: the mockable device-backend layer.
//!
//! Defines the `DeviceBackend` trait, a real-host `CpuBackend`, a `MockBackend`
//! (Vec<u8> "VRAM" with op recording + fault injection + configurable
//! unified-memory flag), and a `GpuCodec` that orchestrates tenso-core
//! encode/decode against a backend and handles GPU IPC reference framing.
//!
//! No real GPU is required: `CpuBackend` treats host memory as "device memory"
//! and implements IPC via a process-local registry so the GPU-direct paths
//! (Jetson unified memory + CUDA IPC) are fully exercisable on macOS/CI.
...
pub struct PinnedBuf {
    /// Host-visible bytes. For the mock/cpu backends this is just a Vec.
    pub bytes: Vec<u8>,
}
...
pub enum DevErr {
    ...
    FaultInjected,
    Core(TensoError),
    /// A backend-specific driver call failed, carrying the native status code
    /// (e.g. a CUDA `cudaError_t`). Distinct from `IpcUnsupported`, which means
    /// the device/platform fundamentally cannot do IPC; `Device(code)` means a
    /// call was attempted and the driver rejected it with `code`.
    Device(i32),
    Other(&'static str),
}
...
// =============================================================================
// CpuBackend (real host memory; IPC via a process-local registry)
// =============================================================================
//
// "Device memory" is an ordinary heap allocation (a boxed Vec<u8>). A DevPtr is
// the raw host address of that buffer's first byte. We track live allocations
// (addr -> (Box, len)) in a global registry so free() can reclaim them and so
// copy/IPC can validate the pointer and length.
//
// IPC is modelled with a process-local handle registry: export_ipc records the
// (addr, len) under a freshly generated 64-byte handle key + the backend's UUID;
// import_ipc looks the handle up and re-validates the UUID. This is testable on
// macOS (no shm, no GPU) while preserving the cross-process semantics: a handle
// only resolves on a backend whose UUID matches the exporter.

struct CpuAlloc {
    // Kept boxed so the backing storage address is stable for the DevPtr.
    storage: Box<[u8]>,
    len: usize,
}
...
struct CpuRegistry {
    ...
    // monotonically increasing seed for synthesising IPC handle bytes
    handle_seed: u64,
}
...
/// Host-memory backend: "device memory" is ordinary host memory, transfers are
/// memcpy, IPC is a process-local handle registry. Reports unified memory.
...
impl CpuBackend {
    pub fn new() -> Self {
        // Distinct, stable per-process UUID so import_ipc can reject handles
        // exported by a backend with a different UUID.
...
impl DeviceBackend for CpuBackend {
    ...
    fn export_ipc(&self, p: DevPtr) -> Result<IpcHandle, DevErr> {
        ...
        // Encode the host address + a uniquifying seed into the opaque handle so
        // the registry lookup is deterministic and collision-free in-process.
...
struct MockSlab {
    // Each allocation is a self-contained Vec acting as fake VRAM.
    data: Vec<u8>,
}

struct MockState {
    // DevPtr.0 is a synthetic 1-based id; the slab map holds the bytes.
    slabs: HashMap<usize, MockSlab>,
    next_id: usize,
    log: Vec<MockOp>,
    // op counter; when it equals `fault_at` the next faultable op fails once.
    op_counter: usize,
    fault_at: Option<usize>,
    // exported handles: handle bytes -> (id, len, uuid)
    ipc: HashMap<[u8; IPC_REF_HANDLE_LEN], (usize, usize, [u8; IPC_REF_DEVICE_UUID_LEN])>,
    handle_seed: u64,
    // configurable capacity (None = unbounded); total bytes currently allocated
    capacity: Option<usize>,
    in_use: usize,
}

/// Mock GPU backend: VRAM is a Vec<u8>, ops are recorded, faults can be
/// injected, and the unified-memory flag is configurable so Jetson-style
/// zero-copy paths are testable without a GPU.
...
impl MockBackend {
    ...
    /// Inject a fault on the Nth (zero-based) faultable operation. The op
    /// counter increments once per faultable backend call (alloc / copy_h2d /
    /// copy_d2h / alloc_pinned / export_ipc / import_ipc).
    ...
    // Returns true (and consumes the fault) if this faultable op should fail.
...
impl DeviceBackend for MockBackend {
    ...
    fn copy_h2d(&self, dst: DevPtr, src: &[u8]) {
        ...
        // Faults on void-returning copies are recorded but cannot surface an
        // error through the trait signature; they leave the slab untouched.
        if Self::check_fault(&mut st) {
            return;
        }
        ...
        match st.ipc.get(&h.bytes) {
            Some(&(id, len, exp_uuid)) => {
                if exp_uuid != self.uuid {
                    ...
                    // A sub-buffer offset past the allocation is invalid.
                    Err(DevErr::InvalidPointer)
                } else {
                    // Honor byte_offset so a sub-buffer IPC reference resolves to
                    // base + offset, not the base pointer (matches the wire field).
...
// =============================================================================
// IpcRef packet framing (device-local mirror of tenso-core's wire layout)
// =============================================================================
//
// We build/parse the 106-byte IpcRef packet directly here from tenso-core's
// framing constants so the device crate's IPC paths are self-contained and do
// not depend on tenso-core's (separately-owned) `write_ipc_ref`/`parse_ipc_ref`
// bodies being filled in. The byte layout is identical, so packets round-trip
// through tenso-core's `parse_ipc_ref`/`decode` and vice-versa.
//
// Layout (all integers little-endian):
//   header[0..10]  = v4 header: magic, ver=4, flags=FLAG_GPU_IPC_REF,
//                    dtype_code=Bytes, ndim=0, reserved=IPC_REF_DISCRIMINATOR
//   body[10..106]  = handle[64], byte_offset:u64, nbytes:u64, device_uuid[16]
...
fn build_ipc_packet(ipc: &IpcRef) -> Vec<u8> {
    ...
    // dtype_code byte (offset 7) is 0 for an IpcRef: tenso-core's authoritative
    // `write_ipc_ref` writes 0 here (via write_v4_header(.., dtype=0, ..)), so we
    // match it byte-for-byte. An IpcRef carries no element dtype of its own.
    out[7] = 0;
    out[8] = 0; // ndim: an IpcRef carries no inline shape
...
fn parse_ipc_packet(bytes: &[u8]) -> Result<IpcRef, TensoError> {
    ...
    // Rule: an IpcRef must never carry an integrity footer.
...
pub struct DecodeResult {
    pub ptr: DevPtr,
    pub nbytes: usize,
    /// True when unified memory let us avoid an H2D copy.
    pub zero_copy: bool,
}

/// Drives tenso-core encode/decode against a `DeviceBackend`, including IPC
/// reference framing (using tenso-core's IpcRef wire layout).
...
impl<'b, B: DeviceBackend> GpuCodec<'b, B> {
    ...
    /// Encode a tensor whose body currently lives in device memory.
    ///
    /// Stages the device bytes back to host (D2H) and feeds them to
    /// tenso-core's dense encoder. Under unified memory the D2H is still
    /// performed for the host-side encoder, but the backend may treat it as a
    /// no-cost view.
    ...
    /// Decode a packet's dense body straight into a caller-provided device
    /// pointer of `dst_capacity` bytes, returning the number of body bytes
    /// written. Errors with `OutOfMemory` if the decoded body would exceed
    /// `dst_capacity` rather than silently truncating (or overflowing real VRAM).
    ...
    /// Decode a packet, allocating device memory (or, under unified memory,
    /// aliasing host memory) and reporting whether the path was zero-copy.
    ///
    /// Under unified memory we still materialise the bytes in a device slab so
    /// the returned `DevPtr` is usable, but flag `zero_copy = true` to reflect
    /// that no discrete H2D staging copy was required on real unified hardware.
    pub fn decode_to_device(&self, bytes: &[u8]) -> Result<DecodeResult, DevErr> {
        let body = self.decode_dense_body(bytes)?;
        self.place_body_on_device(body)
    }

    /// Allocate a device slab for `body` and stage it, reporting `zero_copy`
    /// according to the backend's unified-memory flag. Factored out so the
    /// unified-vs-staged decision is testable without tenso-core's decode body.
    fn place_body_on_device(&self, body: &[u8]) -> Result<DecodeResult, DevErr> {
        ...
        // On real unified hardware the decoded host body and device memory
        // share an address space, so no staging DMA is required; on a discrete
        // device an explicit H2D is needed. In both cases we mirror the bytes
        // into the (mock) slab so the returned DevPtr is dereferenceable, but
        // the `zero_copy` flag records which physical path real hardware takes.
    ...
    pub fn export_ipc(&self, ptr: DevPtr, nbytes: usize) -> Result<Vec<u8>, DevErr> {
        ...
        let ipc = IpcRef {
            ...
            // Prefer the caller-stated size; fall back to the handle's nbytes.
    ...
    pub fn import_ipc(&self, bytes: &[u8]) -> Result<DecodeResult, DevErr> {
        ...
        // Rule: reject device_uuid mismatch at the codec layer too (defence in
        // depth; the backend also re-checks).
        ...
        Ok(DecodeResult {
            ...
            // An imported IPC reference is inherently zero-copy: the memory is
            // shared, never staged.
            zero_copy: true,
        })
    }

    /// Decode `bytes` and borrow the dense body, mapping non-dense / IPC
    /// packets to a device error. Shared by the decode_* paths.
    fn decode_dense_body<'p>(&self, bytes: &'p [u8]) -> Result<&'p [u8], DevErr> {
        match tenso_core::decode(bytes).map_err(DevErr::Core)? {
            ...
            // GpuCodec's device decode paths only handle dense bodies; an
            // IpcRef must go through import_ipc, and structured packets aren't
            // a single contiguous device body.
...
// =============================================================================
// Tests (run with NO GPU; exercise CpuBackend, MockBackend, and the IPC frame)
// =============================================================================
...
mod tests {
    ...
    //
    // We exercise the codec's place-on-device step directly (the same call
    // `decode_to_device` makes after tenso-core decode yields a dense body), so
    // the unified-vs-staged decision is verified at runtime WITHOUT depending on
    // tenso-core's (separately-owned) `decode` body being filled in. The
    // contract is: DecodeResult.zero_copy == backend.is_unified_memory().
